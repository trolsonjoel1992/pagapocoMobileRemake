...
type PieceConditionInputProps = {
  value: string;
  onChangeText: (text: string) => void;
  onValidChange?: (isValid: boolean) => void;
};

const PieceConditionInput = ({
  value,
  onChangeText,
  onValidChange,
}: PieceConditionInputProps) => {
  const { colors, theme } = useTheme();
  const isValid =
    value.trim().toLowerCase() === 'nuevo' ||
    value.trim().toLowerCase() === 'usado';
...
export default PieceConditionInput;
